...
% matrix X (nxn) where det(X)=1. Tangent vectors are symmetric matrices of the same size
% which are trace orthogonal to X^(-1), i.e. tr(X^(-1)*eta)=0.
%
...
%       speed up line-searches.
    ...
    M.name = @() sprintf('Symmetric special linear geometry of %dx%d matrices', n, n);
...
%         M.proj = @(X, eta) symm(eta);
    ...
    function Y = retraction(X, eta, t)
        ...
        % This second order retraction is only usefull for small matrices since the computation
        % of the determinant is expensive and errorprone
